class Rezzable::DonationBox < ApplicationRecord
  ...
  def biggest_donor
    data = self.transactions.group(:target_name, :target_key).sum(:amount).sort_by { |_k, v| v }.last
    {target_name: data.first.first, target_key: data.first.last, amount: data.last}
  end
  
  def donors
    data = self.transactions.group(:target_name, :target_key).sum(:amount).sort_by { |_k, v| -v }
