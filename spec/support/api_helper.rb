...
def hash_key(sending_object, opts)
...
def headers(sending_object = nil, opts = {})
  hash_time = Time.now.to_i
  {
    ...
    'x-auth-digest': api_hash(hash_key(sending_object, opts), hash_time),
    ...
    'x-secondlife-object-key': if opts[:object_key].nil?
                                 sending_object.object_key
                               else
                                 opts[:object_key]
                               end,
